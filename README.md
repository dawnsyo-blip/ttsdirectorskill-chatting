...
一个 Claude Code skill，给中文文本自动插入 [Fish Audio S2 Pro](https://fish.audio/) 的方括号情感/韵律标签（`[soft]`、`[chuckle]`、`[long pause]` 之类），让 TTS 读出来更有语气、更像真人说话。配了一个本地网页 demo，可以一键对比"加了标注的 Fish Audio"和"原文直读的 ElevenLabs"。
...
- 标签选择经常是**反直觉**的：命令句通常要用软标签而不是硬标签，全知型人设不适合用 `[surprised]`，一本正经的冷面吐槽在这个模型上很难读出"嘴硬心软"的反差。
