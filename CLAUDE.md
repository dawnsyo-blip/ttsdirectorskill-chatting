...
- 标签选择经常是**反直觉**的：命令句要用软标签而不是硬标签，deadpan 在这个模型上表现不出"嘴硬心软"的反差。
- **判据必须是标注模型自己能验证的东西**。踩过一次：`[surprised]` 原来的规则是"全知型人设慎用"，但标注时看不到人设说明（而且标注模型和聊天模型是分开的，通常连上一轮对话都拿不到），这条规则要么被忽略、要么被执行成"永远不用"，等于废掉一个标签。改成了"看信息落差方向"——新信息是**证实**还是**推翻**了说话者原本的判断，纯靠文本里的词汇痕迹（"我就知道/果然" vs "真的假的/你居然"）就能判断。**新写规则时先问一句：标注模型手上有没有做这个判断所需的信息？**
- **称呼固定三个词，不要混用**（SKILL.md 开头有术语定义）：**说话者**＝要合成语音的那一方（不用"她/他""角色"）、**对方**＝说话者说话的对象（不用"听者"）、**用户**＝调用 skill 的人。改文档时顺手保持一致。
...
- **唯一一条有意的"鼓励使用"是 `[surprised] [stammering]`，别把它当成疏漏改掉**。理由：`[stammering]` 一直几乎不被触发，因为它只有释义没有触发条件；而 `[surprised]` 经过"信息落差方向"的判据之后本身已经很窄，所以在这道窄门之后鼓励接上结巴，不会造成高频使用。设计上还刻意让**惊讶的强度分档**（`[slightly surprised]` 不接、`[surprised]` 接）同时编码"要不要结巴"，避免引入第二套独立判据——之前写过一版基于"辩解堆叠/句长塌陷/未被问就否认"的失控信号清单，和 `[surprised]` 判据做重复工作，已经删掉了。
...
- **`git push` 也一样不读系统代理**：表现是 `Recv failure: Connection was reset` 或 `Failed to connect to github.com:443 after 21045 ms`。临时解法是在命令前带上环境变量：`HTTPS_PROXY=http://127.0.0.1:1080 git push origin main`（这台机器的代理端口是 1080）。想一劳永逸就 `git config --global http.proxy http://127.0.0.1:1080`。这是继 undici ProxyAgent、npm config 之后**第三套**独立的代理配置，三者互不相通。
